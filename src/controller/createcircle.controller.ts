...
export class CreateController {
    ...
    // 图片存储路径
    private baseImagePath = join(__dirname, '..', 'src', 'circle_data');

    @Post('/createcircle')
    async createCircle(@Body() body: CircleRequestBody): Promise<{ success: boolean; url?: string }> {
        const { circle_name, file } = body;
